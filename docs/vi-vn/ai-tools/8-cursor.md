...
- **Quy mô (theo nguồn tới đầu 2026):** doanh thu annualized vượt **~2 tỷ USD ARR**; được hàng triệu dev dùng; **~64% công ty Fortune 500** dùng Cursor *(số do chính Cursor công bố trên trang Enterprise — xem là marketing claim, không phải kiểm toán độc lập)*.
- **Phiên bản hiện tại (giữa 2026):** **Cursor 3.0** (ra mắt ~tháng 4/2026), giao diện "agent-first", kèm mô hình riêng **Composer 2.5** (~tháng 5/2026). Cursor 3 chuyển sang **"Agents Window"** hợp nhất agent local/cloud/remote, *theo nguồn* hỗ trợ chạy tới ~8 agent song song.
...
| **Composer** | Mô hình agentic độc quyền của Anysphere cho tác vụ coding dài/nhiều bước. **Composer 2.5** (5/2026); *theo Cursor, Composer 2 nhanh ~4x mô hình frontier ngang hàng, đa số lượt agent xong dưới 30 giây*. |
...
| **Multi-model** | Truy cập model của OpenAI, Anthropic (Claude), Google Gemini, xAI Grok + model riêng (Composer). **Auto mode** chọn model tự động. |
:::
...
| Tiêu chí | **Cursor** | **VS Code + Copilot** | **Windsurf → Devin Desktop** | **Claude Code** | **OpenAI Codex** |
|---|---|---|---|---|---|
| Dạng | IDE AI-native (fork VS Code) | IDE + extension | IDE AI-native | Agent terminal (CLI) | Cloud agent tự chủ |
| Mức tự chủ | Trung bình–cao, "bạn lái, agent phụ" → đang dịch sang agent-first | Thấp nhất (gợi ý + chat) | Ở giữa | Cao ("như junior dev") | Cao nhất (chạy nền, trả PR) |
...
| Giá khởi điểm | $20/th (Pro) | **$10/th** — rẻ nhất, free tier rộng | (đang đổi) | $17/th Pro, $100+/th Max | gói cao ~$200/th |
...
- **Delegate refactor lớn / việc nặng kiểu "giao rồi review":** → **Claude Code** (điểm benchmark cao nhất cho tác vụ phức tạp; *theo nguồn, Claude Opus 4.5 dẫn SWE-bench Verified ~80.9% — 5/2026*).
...
> ⚠️ **Lưu ý lớn về Windsurf:** *theo nguồn (~đầu tháng 6/2026)*, Cognition đã **khai tử thương hiệu Windsurf và tái ra mắt thành "Devin Desktop"**. Tin rất mới, nên kiểm tra lại trước khi quyết định.
:::
...
- **Auto mode = KHÔNG giới hạn** — để Cursor tự chọn model, bạn không lo trừ credit.
- **Tự tay chọn frontier model** (ví dụ Claude Opus, GPT đời mới) thì **trừ credit** theo lượt dùng.
...
- **Cursor nhận:** **Visa/Mastercard** và **Alipay** (từ 2024). **KHÔNG** nhận UnionPay / PayPal / WeChat Pay *(theo nguồn thứ cấp)*.
...
- **Cách dev VN thường dùng:**
  ...
  3. **Trả bằng crypto** (BTC/USDT/USDC) qua một số dịch vụ proxy thanh toán.
...
cursor -p "fix the failing tests in src/auth/"
```

```bash
# Kết hợp định dạng output cho script (json hoặc text)
cursor -p "refactor module X" --output-format json
```
...
1. **Để Auto mode khi không cần model cụ thể** → không trừ credit, đỡ lo hoá đơn; chỉ chọn frontier model cho việc thật sự khó.
...
- **Lỗ hổng MCP đã công bố — CurXecute (CVE-2025-54135):** prompt injection gián tiếp khiến Cursor ghi file `.cursor/mcp.json` độc → **remote code execution (RCE)** chỉ bằng một tin nhắn.
...
|---|---|---|
| **"You've reached your request limit" / "Too many requests"** | Hết fast requests; spam ⌘K/regenerate | Chờ ~60s; tránh bắn liên tục. *Theo nguồn, Pro có ~500 fast requests/tháng* — **số này nguồn thứ cấp & hay đổi** |
...
→ **Auto mode không giới hạn / không trừ credit**; tự chọn frontier model thì **trừ credit** theo lượt. Việc thường để Auto, việc khó mới chọn model mạnh.
...
3. Làm một task bằng **Auto mode** (không chọn frontier model) và quan sát credit **không** bị trừ; sau đó thử chọn tay một frontier model và để ý sự khác biệt.

::: details ✅ Tiêu chí hoàn thành
- Rule `.mdc` **được áp dụng** (agent tự bám quy tắc mà bạn không nhắc lại).
- MCP server xuất hiện sau khi khởi động lại Cursor.
- Bạn hiểu **Auto mode (không trừ credit)** vs **chọn tay frontier model (trừ credit)** — và biết khi nào dùng cái nào.
:::
...
2. **Có bản Free (Hobby)** dùng ngay không cần thẻ; nghiêm túc thì **Pro $20/th**. Hiểu rõ **hệ thống credit** (Auto = không trừ; chọn tay frontier model = trừ) — đây là chỗ dễ nhầm & dễ "cháy túi" nhất.
...
> *Tài liệu trong chương dựa chủ yếu trên docs chính thức `cursor.com` (cập nhật tới ~giữa 2026). Các con số hiệu năng/benchmark (Tab ~72%, Composer ~4x, +39% PR, "8 agent song song"…) phần lớn do **Cursor tự công bố** — đã gắn nhãn "theo nguồn". Giá, tên gói và chính sách credit thay đổi nhiều lần; khi nghi ngờ, kiểm tra <https://cursor.com/pricing> trước khi mua.*
