...
### So với công cụ khác (ChatGPT, Manus, Copilot)

Học viên VN gần như chắc chắn sẽ hỏi *“so với ChatGPT thì sao?”*. Khác biệt **cốt lõi** nằm ở chỗ **agent chạy ở đâu** và **có chạm được file trên máy bạn không**. Cowork chạy **VM ngay trên desktop** nên đọc/ghi file local và hiểu sâu document Office phức tạp — đó là **điểm bán riêng** của nó; còn ChatGPT Agent/Manus chủ yếu chạy **trên cloud, không truy cập file local**.

| Tiêu chí | **Claude Cowork** | **ChatGPT Agent / Operator** | **Manus (“My Computer”)** | **Microsoft Copilot** |
|---|---|---|---|---|
| Truy cập **file local**? | **Có** (đọc/ghi máy bạn) | **Không** (chạy VM cloud) | **Không** (cloud sandbox) | **Có** (mạnh với hệ Office) |
| Cloud hay desktop? | **Desktop** (VM local) | **Cloud** | **Cloud** | Desktop + cloud (hệ MS 365) |
| Điểm mạnh | Hiểu sâu Word/Excel/PPT phức tạp nhờ VM local; chuỗi việc trên file | Tác vụ web (đặt vé, điền form); có *“takeover mode”* trả quyền khi cần nhập mật khẩu | Tự chạy task dài kiểu “máy tính ảo riêng” | Tích hợp chặt Office + dữ liệu doanh nghiệp |
| Mô hình tính phí | Theo gói (hạn mức usage) | Theo gói ChatGPT | **Tín dụng (credits)** — khó dự đoán: 1 task phức tạp ngốn **~500–900 credits** | Theo license MS 365 / Copilot |
| Giá khởi điểm | **20 USD/tháng** (Pro) | Theo gói ChatGPT Plus/Pro | Gói trả phí từ **~19 USD/tháng** | Theo gói Copilot doanh nghiệp |
| Có **free tier**? | **Không** | Một phần (tùy tính năng) | **Có** (giới hạn) | Một phần (tùy license) |

::: warning 🧭 Đọc bảng này thế nào
Bảng so sánh ở **mức định hướng** (theo tài liệu công khai tới giữa 2026) — các sản phẩm này thay đổi rất nhanh, **tự kiểm chứng** khi chọn mua. Manus do **Butterfly Effect** phát triển (theo báo chí, đã được Meta mua lại cuối 2025); bản *“My Computer”* ra mắt khoảng 3/2026. Mô hình **credits của Manus** đặc biệt khó dự đoán chi phí — điểm cần lưu ý nếu bạn nhạy giá.
:::
...
::: warning ⚠️ Giới hạn thật — đọc trước khi mua
Trước khi quyết chi tiền, biết 4 giới hạn hay làm người mới *vỡ mộng*: **(1)** không có Free tier — tối thiểu Pro 20 USD/tháng; **(2)** **đốt hạn mức cực nhanh**, việc nặng có thể cạn quota trong vài tác vụ; **(3)** bảng tính “trình bày” (ô gộp/nhiều header) và **thao tác trình duyệt nhiều bước** dễ lỗi/chậm; **(4)** ở **VN** chưa nằm trong danh sách quốc gia hỗ trợ chính thức (xem mục 02). Chi tiết các lý do người dùng **bỏ Cowork** nằm ở **mục 06** — nên đọc trước khi xuống tiền.
:::
...
|---|---|---|---|
| **Pro** | **20 USD/tháng** (≈ **17 USD/tháng** nếu trả năm, ~200 USD trả trước) | Đầy đủ tính năng, **usage thấp nhất** | Dùng nhẹ / thỉnh thoảng |
...
| **Team / Enterprise** | Team ~**20 USD/seat/tháng** (trả năm) hoặc ~**25 USD/seat** (trả tháng), **tối thiểu 5 seat**; Enterprise theo thỏa thuận (~20 USD/seat + usage theo API) | Quản trị tập trung | Doanh nghiệp (RBAC, spend cap, analytics…) |
...
**Giảm đau hạn mức:** theo tài liệu tới giữa 2026, các gói trả phí có **“extra usage toggle”** — khi hết hạn mức included, bạn có thể chọn **tiếp tục ở mức giá API với một spend-cap tự đặt** thay vì bị dừng cứng. Hữu ích để không bị kẹt giữa một việc đang chạy dở, nhưng nhớ **đặt trần chi tiêu** kẻo hóa đơn vượt dự kiến.
:::
...
1. **Việt Nam KHÔNG nằm trong danh sách quốc gia được hỗ trợ chính thức** của Claude/Anthropic — *tại thời điểm biên soạn tài liệu này (giữa 2026), VN không có trong danh sách hỗ trợ; danh sách có thể được mở rộng nên bạn vẫn nên tự kiểm chứng* → việc **đăng ký tài khoản & thanh toán có thể bị chặn theo khu vực**.
...
Mở [anthropic.com/supported-countries](https://www.anthropic.com/supported-countries) và kiểm tra Việt Nam có trong danh sách không. Đừng mua gói trước rồi mới phát hiện không kích hoạt được.
:::
...
::: warning 🧱 Sandbox bảo vệ gì — và KHÔNG bảo vệ gì
Sandbox **giới hạn truy cập filesystem** (chỉ folder bạn cấp) và **hạn chế network**, nhưng nó **whitelist chính API của Anthropic là “đáng tin”**. Đó **chính là kẽ hở** mà vụ PromptArmor khai thác (xem hộp “lethal trifecta” ở mục 04): kẻ tấn công không cần mở một kết nối lạ — chúng lừa Cowork **gửi dữ liệu của bạn qua đúng kênh API Anthropic vốn được cho phép**. Nói cách khác: sandbox chặn được “đào tường ra ngoài”, **nhưng không chặn được dữ liệu rò qua một cánh cửa vốn đang mở**. Vì vậy *least-privilege ở cấp folder vẫn là tuyến phòng thủ thực tế nhất của bạn* — sandbox không thay thế cho việc **không bỏ dữ liệu nhạy cảm vào tầm với của agent**.
:::
...
6. **Tài liệu cũ gây hiểu nhầm.** Một số hướng dẫn viết thời preview ghi *“chỉ macOS”* hoặc *“chỉ Max”*. Lưu ý: *“chỉ Max”* **đúng ở bản preview đầu (12/1/2026)**, nhưng từ **16/1/2026** đã mở cho Pro, và bản GA hỗ trợ **cả Windows** và **mọi gói trả phí** — nên các mốc “chỉ Max / chỉ macOS” nay đã **LỖI THỜI**.
...
::: warning 🔐 Dữ liệu của bạn đi đâu? — chính sách train & quyền riêng tư
Đây là câu hỏi **khác** với prompt injection ở trên: prompt injection là *rủi ro bị tấn công*, còn đây là *chính sách dữ liệu* của Anthropic. Hai điểm cần biết (theo chính sách công khai tới giữa 2026 — **tự kiểm chứng vì chính sách thay đổi**):

- **Có gửi lên cloud không?** Cowork chạy VM **trên máy bạn**, nhưng **suy luận của model vẫn diễn ra trên cloud Anthropic** — tức nội dung Claude cần “đọc để xử lý” **vẫn được gửi tới máy chủ**. “Chạy local” **không** có nghĩa là dữ liệu không rời máy.
- **Có bị dùng để train không?** Khác nhau theo loại tài khoản: bản **Team/Enterprise mặc định KHÔNG dùng dữ liệu của bạn để huấn luyện model**; bản **consumer (Free/Pro/Max)** thì **tùy thiết lập** — Anthropic có tùy chọn cho phép/không cho phép dùng dữ liệu cải thiện model, **bạn nên vào Settings kiểm tra và tắt nếu cần**.

Quy tắc thực dụng cho dữ liệu khách hàng/bí mật ở VN: **ưu tiên tài khoản tổ chức (Team/Enterprise) nếu xử lý dữ liệu nhạy cảm**, và luôn **đọc setting quyền riêng tư trước khi đổ dữ liệu thật vào**.
:::
...
**Demo thật:** chỉ **hai ngày sau khi Cowork ra mắt bản Research Preview (tháng 1/2026)** — KHÔNG phải sau bản GA tháng 4 — nhóm **PromptArmor** trình diễn một **file Word chứa prompt injection ẩn** lừa Cowork **upload file nhạy cảm** (gồm tài liệu tài chính có một phần **số an sinh xã hội**) lên **tài khoản Anthropic của kẻ tấn công** — khai thác được vì VM của Cowork **whitelist chính API của Anthropic là “đáng tin”**. Simon Willison nhấn mạnh: bảo người dùng không-rành-kỹ-thuật **“tự canh prompt injection”** là **vô lý**.

**Lỗ hổng sâu hơn “một file Word độc”:** theo các báo cáo, gốc rễ là một **lỗi isolation trong môi trường code-execution của Claude đã được biết TRƯỚC khi Cowork ra đời** — researcher Johann Rehberger từng disclose một dạng tương tự trên Claude.ai, Anthropic ghi nhận nhưng (theo báo cáo, tới giữa 2026) chưa fix triệt để. Đây là **vấn đề mang tính hệ thống**, không phải sự cố lẻ. Kỹ thuật giấu lệnh độc cũng cho thấy vì sao **mắt thường gần như bất khả thi phát hiện**: chữ cỡ **font 1pt, màu trắng trên nền trắng, giãn dòng 0.1**.
...
*Nguồn: PromptArmor (gốc) — `https://www.promptarmor.com/resources/claude-cowork-exfiltrates-files`; The Decoder — `https://the-decoder.com/claude-cowork-hit-with-file-stealing-prompt-injection-days-after-anthropics-launch/`; byteiota.com, mintmcp.com, wonderingaboutai.substack.com, simonwillison.net.*
:::
...
|---|---|---|
| “Mới chạy vài việc đã hết hạn mức” | Mỗi tác vụ đốt token gấp hàng chục lần chat | Canh sau khi reset 5h; cân nhắc Max 5x; cân nhắc bật **extra usage** (có spend-cap) |
...
| Task lớn **cứ phải bấm “Continue”** | Giới hạn **tool-call mỗi lượt** | Chia nhỏ task; với batch lớn cân nhắc **Claude Code** |
| **Scheduled task không chạy** | Laptop **ngủ** / app đóng | Giữ máy thức + app mở; hoặc chấp nhận “catch-up” khi mở lại |
...
**Cowork có bản miễn phí không?** — **Không.** Tối thiểu **Pro 20 USD/tháng**. Bất kỳ trang nào nói “Free includes Cowork” là **đọc nhầm** trang pricing.

**“Chạy local” nghĩa là dữ liệu không rời máy?** — **Không.** VM chạy trên máy bạn, nhưng **suy luận của model vẫn trên cloud Anthropic** → nội dung cần xử lý vẫn được gửi đi. Xem hộp *“Dữ liệu của bạn đi đâu?”* ở mục 04.

**Vì sao task lớn cứ bắt bấm “Continue”?** — Do **giới hạn tool-call mỗi lượt**. Chia nhỏ task; batch rất lớn (100+ file) cân nhắc **Claude Code** (cộng đồng báo Code xong trong ~5 phút so với “babysit” 20 phút trên Cowork).

**Scheduled task không chạy đúng giờ?** — Scheduler **chỉ chạy khi máy thức + app mở**; laptop ngủ thì task **bị skip** (chỉ catch-up lần mở app sau). Giữ máy thức hoặc chấp nhận trễ.

**Chuyển docx ↔ pdf bị lỗi/thiếu chữ?** — Thường do **thiếu công cụ hệ thống**. Cài **LibreOffice + Ghostscript** rồi chạy lại.

**Bảng tính ra lệch ô?** — File nguồn dạng *“trình bày”* (ô gộp, nhiều header) làm skill **xlsx** vật lộn. Giữ layout **đơn giản dạng cột** và kiểm tra lại tay.

**Cowork không thấy file tôi vừa kéo vào?** — File **phải nằm sẵn trong folder** Cowork được trỏ tới; nó **không đọc file kéo từ Downloads**. Chạy qua **browser extension thì chậm + tốn token**.

**“Chỉ Max / chỉ macOS” — còn đúng không?** — **Không.** Đúng ở bản preview đầu (1/2026); bản **GA hỗ trợ Windows và mọi gói trả phí**.

**Dùng ở Việt Nam được không?** — Tới giữa 2026, **VN không có trong danh sách quốc gia hỗ trợ** → đăng ký/thanh toán có thể bị chặn. Tự kiểm chứng vì danh sách có thể mở rộng.
:::

::: tip 🔗 Nguồn chính thức (first-party — kiểm khi cần)
- Trang sản phẩm: [anthropic.com/product/claude-cowork](https://www.anthropic.com/product/claude-cowork)
- Quốc gia hỗ trợ (quan trọng cho VN): [anthropic.com/supported-countries](https://www.anthropic.com/supported-countries)
- Giá & gói: [claude.com/pricing](https://claude.com/pricing)
- Tải Claude Desktop: [claude.com/download](https://claude.com/download)
- Showcase doanh nghiệp: [claude.com/customers](https://claude.com/customers)
- Vụ bảo mật (nguồn gốc PromptArmor): [promptarmor.com/resources/claude-cowork-exfiltrates-files](https://www.promptarmor.com/resources/claude-cowork-exfiltrates-files)
- Báo cáo độc lập vụ bảo mật: [the-decoder.com — Claude Cowork prompt injection](https://the-decoder.com/claude-cowork-hit-with-file-stealing-prompt-injection-days-after-anthropics-launch/)
:::
