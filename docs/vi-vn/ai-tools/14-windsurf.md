...
description: 'Hướng dẫn thực chiến Windsurf — từ 2/6/2026 đổi tên thành Devin Desktop (Cognition), agent local mặc định nay là Devin Local. AI-native IDE fork VS Code: Cascade/Devin Local, Tab/Supercomplete, Codemaps, ACP, Rules/Workflows, MCP, model SWE-1.5/1.6. Cài đặt, giá & quota mới, dùng ở VN, workflow từng bước, case study và lỗi hay gặp.'
---
...
- **Quy mô (theo nguồn chính thức tới giữa 2025):** khoảng **1 triệu+ người dùng** (hàng trăm nghìn DAU), **350+ khách hàng enterprise**, ARR ~**82 triệu USD** (tại thời điểm Cognition mua, 7/2025). Một customer story với Anthropic ghi hệ thống xử lý **100 triệu+ token/phút**.
...
| **SWE-1.5 / SWE-1.6** | Model coding riêng của Cognition. SWE-1.5 là bản GA hiện hành (~950 tok/s); SWE-1.6 (công bố 1/3/2026) hiện vẫn **early preview**. Chi tiết benchmark xem hộp 📊 bên dưới. |
| **ACP (Agent Client Protocol)** (Devin Desktop, 6/2026) | Giao thức cho phép chạy **agent khác ngay trong Devin Desktop** — ví dụ OpenAI Codex, Claude Agent, OpenCode. Đây là điểm "agent-neutral" mà Cursor chưa có. *(Theo blog Cognition/Devin tới giữa 2026.)* |
...
| **Rules / Memories / Workflows** | Quy tắc project đặt trong thư mục **`.devin/rules/*.md`** (ưu tiên cao nhất) hoặc **`.windsurf/rules/*.md`** — file đơn `.windsurfrules` cũ vẫn chạy nhưng **đã deprecated từ Wave 8** (xem mục 03). **Memories** (AI tự ghi nhớ qua phiên), **Workflows** (kịch bản lặp lưu thành slash command). |
...
| **Model riêng** | SWE-1.5 (~950 tok/s); SWE-1.6 đang preview | Composer | GPT/Claude qua Copilot | Opus/Sonnet (Anthropic) | GPT-5-class (gpt-5-codex) |
| **Điểm mạnh** | Codemaps, autonomy, đa-IDE, compliance, **ACP** (chạy được agent khác như Codex/Claude Agent ngay trong IDE) | Snappy, control, team/power user | Hệ sinh thái sẵn, giá rẻ Copilot | Refactor lớn, chất lượng output | Parallel nhiều agent trong VM cloud |
...
- **Model SWE-1.5 của Cognition:** đạt **~40,08% SWE-bench Pro** (theo Scale SWE-bench Pro) và tốc độ ~**950 token/giây**, chạy trên hạ tầng Cerebras. Blog gốc của Cognition tuyên bố SWE-1.5 **nhanh hơn ~13× so với Claude Sonnet 4.5** (và ~6× so với Haiku 4.5) — "near-SOTA về chất lượng nhưng cực nhanh" để vòng lặp agent mượt. Con số điểm Sonnet 4.5 ~43,6% là *từ Scale leaderboard (nguồn thứ cấp, không có trong blog Cognition)* — đặt cạnh để tham khảo, đừng coi cả hai số đến từ cùng một nguồn. *(Nguồn chính: cognition.ai/blog/swe-1-5.)*
- **SWE-1.6 (công bố 1/3/2026):** hiện **vẫn early preview** (rollout cho nhóm nhỏ, **chưa GA cho mọi user**), điểm **+~11% so với SWE-1.5 trên SWE-bench Pro**, vẫn ~950 tok/s; có ghi nhận lỗi "overthinking/self-verification thừa". Đừng cho rằng SWE-1.6 đã mặc định cho tài khoản Pro. *(Nguồn: cognition.ai/blog/swe-1-6-preview.)*
- **Lưu ý lộ trình agent:** từ **2/6/2026** (cùng ngày rebrand, ship qua OTA), **Devin Local đã là agent local mặc định** — bản viết lại bằng Rust, theo Cognition tiết kiệm tới ~30% token, hỗ trợ subagent + sandboxing. **Cascade chỉ còn dùng được như fallback tới 1/7/2026** (sau mốc này không còn nữa). Tức đây không phải kế hoạch tương lai — nếu UI của bạn vẫn ghi "Cascade", bạn đang ở chế độ fallback. *(Nguồn: docs.devin.ai/desktop/devin-desktop-faq.)*
:::
...
- **Lo ngại biến động thương hiệu/sản phẩm:** vừa rebrand thành Devin Desktop (2/6/2026), agent local đã đổi từ Cascade sang **Devin Local** (mặc định từ 2/6, Cascade hết hỗ trợ 1/7/2026) — team cần ổn định nên theo dõi sát hoặc chờ.
- **Bạn vừa onboard và cần tài liệu/tutorial ổn định:** vì agent vừa đổi (Cascade → Devin Local, 2/6) và format rules đổi sang `.devin/rules/`, **nhiều tutorial/screenshot trên mạng — kể cả các bước trong chương này — sẽ lệch với UI thực tế** trong vài tháng tới. Hãy luôn đối chiếu docs chính thức (mục 07).
:::
...
| **Pro** | **20 USD/tháng** | Quota cao hơn; full model (OpenAI, Claude, Gemini); SWE-1.5 (SWE-1.6 khi mở preview) + model open-source miễn phí; Devin Cloud; mua thêm usage theo giá API |
| **Max** | **200 USD/tháng** | Như Pro nhưng quota **cao hơn nhiều** |
| **Teams** | **40 USD/seat/tháng** (tối đa 200 ghế) | Mọi thứ trong Pro + **Zero Data Retention tự động**, admin dashboard + analytics, billing tập trung, priority support; mua thêm credit pooled (~120 USD/1000 credits) |
...
**Bước 2 — Viết bộ quy tắc dự án (rules).** Đây là nơi bạn "dạy" agent về coding standards, kiến trúc, lệnh build, giới hạn **≤ 12.000 token**.

::: warning ⚠️ File `.windsurfrules` đã DEPRECATED — dùng thư mục `.devin/rules/`
File đơn `.windsurfrules` ở gốc repo **vẫn chạy** (giữ tương thích ngược) nhưng **đã deprecated từ Wave 8**. Cách khuyến nghị hiện nay (hợp với rebrand Devin Desktop):
- **`.devin/rules/*.md`** — ưu tiên cao nhất, đúng với Devin Local. **Nên dùng cái này.**
- **`.windsurf/rules/*.md`** — fallback nếu chưa có thư mục `.devin/rules/`.

Mỗi rule là một file `.md` riêng trong thư mục, dễ quản lý hơn một file khổng lồ. Ví dụ dưới mình minh hoạ bằng `.windsurfrules` cho gọn, nhưng bạn nên đặt nội dung này vào `.devin/rules/coding-standards.md`. *(Nguồn: docs.windsurf.com — rules; FAQ Devin Desktop xác nhận `.devin/rules/` được ưu tiên.)*
:::

```text
# .devin/rules/coding-standards.md (khuyến nghị)
# — hoặc .windsurfrules ở gốc repo (cũ, đã deprecated nhưng vẫn chạy)
...
- **Vị trí file:** `.windsurf/workflows/*.md` (commit theo repo, cả team dùng chung) **hoặc** global `~/.codeium/windsurf/global_workflows/*.md`. *(Path global `.codeium` là di sản Codeium; sau 2 lần rebrand có thể đã đổi — kiểm lại trên docs.devin.ai nếu không tìm thấy.)*
...
1. **Luôn viết rules (`.devin/rules/*.md`) trước khi giao việc lớn.** Không có nó, agent đoán mò coding convention của bạn — dễ sửa lệch chuẩn. (File `.windsurfrules` cũ vẫn chạy nhưng đã deprecated — xem mục 03.)
...
- **Zero Data Retention (ZDR)** là **mặc định cho tier Team/Enterprise**: code gửi đi để inference **không lưu** sau khi request xong; tính năng lưu artifact phải **opt-in**. Trang giá chính thức (`devin.ai/blog/windsurf-pricing-plans`) liệt kê thẳng *"Automated zero data retention"* cho gói Teams.
- **Free/Pro:** có thể tắt telemetry ở **Settings → Telemetry → Disable**. *Theo các nguồn thứ cấp (chưa có xác nhận chính thức rõ ràng)*, nếu **không opt-out**, code có thể bị giữ làm dữ liệu training — nên với code nhạy cảm, cân nhắc tier Team/Enterprise (ZDR) hoặc tắt telemetry. Muốn chắc, đọc trực tiếp `windsurf.com/security`.
...
**Hỏi: Cascade với Devin Local — cái nào đang chạy?**
→ Theo FAQ chính thức (docs.devin.ai/desktop/devin-desktop-faq): từ **2/6/2026, Devin Local đã là agent local mặc định** (viết lại bằng Rust, tiết kiệm tới ~30% token, hỗ trợ subagent + sandboxing). **Cascade chỉ còn dùng được như fallback tới 1/7/2026** — sau đó không còn. Tức nếu hôm nay bạn mở app, mặc định bạn đang dùng Devin Local; FAQ ghi nó giữ "cùng năng lực và cài đặt như Cascade" nên 3 chế độ Write/Chat/Plan nhiều khả năng giữ nguyên tên.
:::
...
**Mục tiêu:** trải nghiệm vòng *agentic* của Cascade: lập kế hoạch → sửa nhiều file → chạy test → tự sửa. Có file rules để bám chuẩn.

1. Tạo file rules `.devin/rules/coding-standards.md` (hoặc `.windsurfrules` cũ ở gốc repo — theo mẫu mục 03, Bước 2).
...
2. **Có bản Free** (Tab + inline edit không giới hạn, quota agent nhẹ); **Pro 20 USD/tháng**, **Max 200 USD**, **Teams 40 USD/seat** (tối đa 200 ghế, kèm ZDR tự động).
...
5. **Quy trình chuẩn:** Open Folder → viết rules trong `.devin/rules/*.md` (file `.windsurfrules` cũ đã deprecated) → agent (Plan cho task lớn / Write để sửa / Chat để hỏi) → duyệt diff → đóng việc lặp thành Workflow `.windsurf/workflows/*.md`.
...
Sản phẩm đổi rất nhanh (vừa rebrand thành Devin Desktop, agent local đã chuyển sang Devin Local) — khi giáo trình lỗi thời, dùng các link chính thức sau để tự cập nhật. *(Lưu ý: một số link `windsurf.com/...` đang dần 301 sang `devin.ai`.)*
...
| FAQ Devin Desktop (rebrand, Cascade→Devin Local) | <https://docs.devin.ai/desktop/devin-desktop-faq> |
