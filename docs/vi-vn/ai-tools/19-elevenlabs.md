...
- **Series D** (04/02/2026): **500 triệu USD, định giá 11 tỷ USD** — Sequoia dẫn (a16z gấp 4, ICONIQ gấp 3, thêm Lightspeed/BOND). ElevenLabs được CNBC mô tả là *"Nvidia-backed"* (Nvidia là nhà đầu tư chiến lược **từ trước**, không nằm trong danh sách vòng D theo *TechCrunch*) và *"đang nhắm IPO"* (cách diễn đạt của CNBC, chưa phải tuyên bố chính thức) — *TechCrunch / CNBC*.
...
| Tiêu chí | ElevenLabs | OpenAI TTS | PlayHT *(đã đóng cửa 31/12/2025)* | Murf | Google / Azure TTS |
|---|---|---|---|---|---|
...
| **Số giọng dựng sẵn** | Rất nhiều (~1.200+ *theo nguồn so sánh*) | **~9–11 giọng** *(ít hơn nhiều; theo docs OpenAI)* | Nhiều | Theo use-case | Nhiều, đa accent |
...
| **Giá tham khảo** *(hedge — đổi liên tục)* | Đắt hơn theo credit (≈ ký tự); Flash ~½ giá | tts-1 ~$15 / 1M ký tự | (không còn bán) | Theo gói seat | ~$4–16 / 1M ký tự |
...
**PlayHT** — đối thủ TTS quen thuộc — **đã bị Meta mua (12/07/2025)** và **API PlayHT đóng cửa 31/12/2025**. Nó vẫn nằm trong bảng trên **chỉ để đối chiếu lịch sử**, không còn là lựa chọn mua được. Nếu bạn (hoặc tool bạn theo dõi) còn xài PlayHT, hãy lên kế hoạch chuyển; ElevenLabs là một trong những điểm đến chính của làn sóng người dùng PlayHT.

**Đối thủ low-latency đáng để mắt sau khi PlayHT chết: [Cartesia](https://cartesia.ai) (model Sonic)** — được nhắc cùng ElevenLabs/Murf như alternative chính cho nhóm cần độ trễ thấp (real-time/voice agent).
:::

::: tip 💡 Tóm tắt nhanh
- Cần **giọng chân thực nhất + clone + dub** → **ElevenLabs**.
- Đã sống trong **hệ sinh thái OpenAI** và chỉ cần TTS đơn giản → **OpenAI TTS** (nhưng không có clone, chỉ ~9–11 giọng).
...
- **Đã ở sâu trong hệ sinh thái OpenAI** và chỉ cần TTS đơn giản → **OpenAI TTS** tiện hơn (dù không clone, chỉ ~9–11 giọng).
...
ElevenLabs vào được từ VN **không cần VPN**, có giao diện và **giọng tiếng Việt chính thức** ([elevenlabs.io/text-to-speech/vietnamese](https://elevenlabs.io/text-to-speech/vietnamese)). Tiếng Việt được hỗ trợ từ ~2024 (đợt mở rộng từ 29 → 32 ngôn ngữ, thêm Vietnamese/Hungarian/Norwegian); cả **Flash v2.5** và **Turbo v2.5** đều chạy tiếng Việt (ElevenLabs khuyến nghị **dùng Flash thay Turbo** — Flash đã vượt Turbo về độ trễ/giá). Speech-to-Text nhận diện **3 accent: Bắc (Hà Nội/chuẩn), Trung (Huế), Nam (TP.HCM)**.
...
- **1 ký tự = 1 credit** với model chất lượng cao như **Multilingual v2**.
- Model **Flash / Turbo v2.5 rẻ hơn — ~0.5 credit/ký tự** (giảm tới ~50% giá qua API). Việc real-time/khối lượng lớn nên ưu tiên Flash.
- **v3:** hệ số credit có thể **khác Multilingual v2 tùy thời điểm** (v3 từng có giai đoạn khuyến mãi giảm giá mạnh quanh lúc ra GA) — đừng đoán cứng 1:1, hãy kiểm tra [pricing/api](https://elevenlabs.io/pricing/api) live trước khi chạy số lượng lớn.
- Credit dư **rollover tối đa ~2 tháng** nếu bạn còn gói trả phí.
- Mẹo: ước lượng nhanh — một video 8 phút lời ≈ 8.000–10.000 ký tự ≈ 8.000–10.000 credit (Multilingual v2) hoặc ~một nửa (Flash).
:::
...
- SDK `elevenlabs` **đổi API surface giữa các bản major** → cài bản mới (`pip install -U elevenlabs`) để khỏi copy nhầm code cũ.
:::
...
2. **Cắt văn bản dài thành đoạn ~5 phút** rồi ghép — vừa né giới hạn ký tự, vừa tránh giọng "trôi" giữa chừng. **Mẹo audiobook:** Flash v2.5 cho tới **~40.000 ký tự/request** (gấp ~8x Multilingual v2, ~8x v3) → khối lượng lớn dùng Flash không chỉ vì rẻ mà vì **chịu đoạn dài hơn hẳn**, đỡ phải chia nhỏ.
...
- **Giới hạn độ dài/request (theo docs):** ~5.000 ký tự cho v3, ~10.000 cho Multilingual v2, nhưng **~40.000 cho Flash v2.5** → podcast/audiobook dài: hoặc chia nhỏ, hoặc dùng Flash để chịu đoạn dài hơn.
...
**(c) Dữ liệu giọng nói:** ElevenLabs nói **không dùng voice data/personal data để profiling/targeting**; **không giữ dữ liệu giọng của bạn quá ~3 năm** sau lần tương tác cuối (trừ khi luật yêu cầu — *con số retention hay đổi, double-check ở Privacy Policy hiện hành*). **Mặc định không train trên dữ liệu khách Enterprise.**
...
Trust Center: **compliance.elevenlabs.io** — danh mục chứng chỉ (nhất là ISO 42001, loại mới) cập nhật liên tục, **tự kiểm tại đây** trước khi đưa vào hồ sơ tuân thủ doanh nghiệp.
:::
...
- **Tài liệu API / quickstart:** https://elevenlabs.io/docs/quickstart (tổng quan model: https://elevenlabs.io/docs/overview/models)
...
- **"~1.200+ giọng" của ElevenLabs:** từ blog so sánh bên thứ ba, **không** phải số chính thức ElevenLabs. Số giọng OpenAI TTS là **~9–11** (alloy, ash, coral, echo, fable, onyx, nova, sage, shimmer… theo docs OpenAI tới giữa 2026) — con số "~6 giọng" ở các bài so sánh cũ đã lỗi thời.
...
- **Mốc của Eleven v3:** alpha **05/06/2025**, GA chính thức **02/02/2026** (blog *"Eleven v3 is Now Generally Available"*).
- Tin tài chính: Series C ($180M @ 3,3 tỷ USD, 01/2025) & Series D ($500M @ 11 tỷ USD, 02/2026; **tổng vốn ~781 triệu USD qua 5 vòng** theo *TechCrunch*) — *TechCrunch / CNBC*; PlayHT bị Meta mua (07/2025), API đóng 31/12/2025.
