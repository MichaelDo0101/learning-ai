...
Theo nguồn báo chí (TechCrunch, Bloomberg, Sacra) tới giữa 2026 — chú ý đây là **hai mốc gọi vốn cách nhau ~6 tháng**, đừng gộp làm một:

- **09/2025:** huy động **~250 triệu USD**, định giá **~3 tỷ USD** (ARR ~150 triệu USD annualized).
- **03/2026:** huy động thêm **~400 triệu USD**, định giá nhảy lên **~9 tỷ USD**.

Doanh thu 2025 được các nguồn ước tính dao động mạnh **~150–300 triệu USD ARR** (số private, không kiểm toán). Mục tiêu **1 tỷ USD ARR** cuối 2026. *Đây là số liệu báo chí, không phải số kiểm toán công khai — chỉ dùng để hình dung quy mô, đừng trích như sự thật tài chính chính thức.*
:::
...
| Giá tham chiếu | Free + Core ~$20–25 + Pro $100 (credit) | Pro $20/th (gồm $20 credit, vượt tính theo dùng); còn Pro+ $60, Ultra $200, Teams $40/user | Theo gói ChatGPT + dùng | Free / Pro $25 / Teams $30/member (tính theo **token**) | $0.18/giờ (2 core); free 120 core-hours (≈60 giờ máy 2-core) / 15GB |
...
- **App cần tải nặng / đồng thời cao, latency thấp ổn định ở production** → cân nhắc hạ tầng chuyên dụng, không chỉ Autoscale của Replit.
...
- **Core giảm từ $25 → $20** (trả năm); Core hiện hữu bị tính giá mới từ kỳ gia hạn kế tiếp sau **25/02/2026**.
- **Pro ra mắt 20/02/2026**, thay thế gói **Teams** cũ — khách Teams được tự nâng lên Pro (rollout từ **03/03/2026**).
...
- Task đơn giản (sửa nhỏ) có thể **dưới $0.25** — blog Replit nêu mức **~$0.06** cho task rất đơn giản.
- Task phức tạp có thể tốn **tới vài USD** (agent làm nhiều, chạy nhiều test, suy luận lâu → tốn hơn). Dải chính thức từ blog Replit: **~$0.06 → vài USD/task**.
...
### Phí deployment tính RIÊNG — chọn đúng loại deployment

Đây là chỗ người dựng app thật hay vấp: **chi phí deploy app chạy 24/7 nằm TÁCH KHỎI credit Agent**. Khi bấm "deploy", Replit hỏi bạn chọn **loại deployment** — mỗi loại tính tiền và hành xử khác nhau *(theo tài liệu Replit tới giữa 2026)*:

| Loại deployment | Khi nào dùng | Đặc điểm chi phí |
|---|---|---|
| **Autoscale** | Web app/API lưu lượng thay đổi | **Scale-to-zero** (ngủ khi không ai dùng → có thể "lạnh" lúc gọi lại), trả theo request/tài nguyên |
| **Reserved VM** | App cần **always-on**, latency ổn định, có background job | Máy chạy liên tục, **phí cố định/tháng** dù có người dùng hay không |
| **Static** | Trang tĩnh (HTML/JS, landing) | Rẻ nhất, không chạy backend |
| **Scheduled** | Cron job chạy theo lịch | Trả theo lần chạy |

::: warning ⚠️ "App deploy xong nhưng tự tắt/ngủ khi không ai dùng"
Đó thường là do bạn đang dùng **Autoscale** (scale-to-zero) — đúng theo thiết kế để tiết kiệm tiền. Nếu cần app **luôn bật** (vd có cron nội bộ, hoặc demo cho khách bất chợt), chọn **Reserved VM**. Đừng nhầm "app hỏng" với "app đang ngủ".
:::
...
- **Nội dung private không dùng để train AI** (cam kết của Replit — điểm bán cho doanh nghiệp). *Lưu ý: SOC 2 không kiểm soát việc training — điều khoản "không train" nằm ở **DPA/Terms**, không phải trong SOC 2 report; doanh nghiệp nhạy cảm nên đọc kỹ DPA.*
...
**Hỏi: Agent sửa file tôi không muốn, hoặc phá phần code đang chạy ngon?**
→ Trước khi giao việc, **rollback về checkpoint** là phanh chính. Ngoài ra: mô tả rõ "chỉ sửa file X, đừng đụng phần Y"; chia app thành phần nhỏ để khoanh vùng; dùng `.gitignore`/connect GitHub để có thêm một lớp version control ngoài checkpoint của Replit.

**Hỏi: App deploy xong nhưng tự ngủ/tắt khi không ai dùng?**
→ Đó là **Autoscale (scale-to-zero)** — đúng theo thiết kế để tiết kiệm tiền, lần gọi đầu sau khi ngủ có thể chậm ("cold start"). Cần app luôn bật thì chọn **Reserved VM** khi deploy (xem bảng loại deployment ở mục 02).

**Hỏi: Làm sao export code ra GitHub / chạy ở máy local? Có bị lock-in không?**
→ Replit cho **connect GitHub** để đẩy code ra repo của bạn (rồi clone về chạy local) — code là của bạn, không khoá cứng trong Replit. Đây cũng là cách backup và review qua PR.
...
Theo Replit, Rokt **build 135 app nội bộ trong 24 giờ**, với **700+ nhân viên tham gia**, nay xử lý **30.000+ task/năm**. **Bài học:** khi mở khả năng "tự dựng tool" cho cả tổ chức, số lượng tool nhỏ giải quyết được tăng vọt. *Nguồn: replit.com/customers/rokt.*
