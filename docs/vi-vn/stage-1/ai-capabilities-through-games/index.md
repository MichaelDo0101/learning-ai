...
Đầu tiên, mở [ChatGPT](https://chatgpt.com/) — trợ lý AI của OpenAI, hiện là một trong những công cụ AI phổ biến và dễ tiếp cận nhất tại Việt Nam. Ngoài việc trả lời câu hỏi, ChatGPT còn có thể **viết code và chạy thử ngay trong trình duyệt** qua tính năng **Canvas**: bạn mô tả ứng dụng muốn làm bằng tiếng Việt, ChatGPT sinh code rồi hiện luôn bản preview chạy được ngay bên cạnh. Ở đây ta sẽ dùng nó để làm một game Snake (tài khoản miễn phí là đủ để làm theo bài này).
...
Mở ChatGPT, gõ yêu cầu của bạn vào khung chat rồi gửi:

![](images/chatgpt-snake-1-prompt.png)
...
Gửi xong, ChatGPT vừa giải thích vừa tự viết code (HTML/CSS/JavaScript) cho bạn. Với một game nhỏ như Snake, thường chỉ khoảng một phút là xong — đúng bằng thời gian pha ly cà phê!

![](images/chatgpt-snake-2-result.png)

Code được mở trong khung **Canvas** ngay bên phải khung chat. Bấm nút phóng to ở góc trên Canvas để xem **toàn màn hình**, rồi bấm **Start** và dùng phím mũi tên chơi thử ngay.

![](images/chatgpt-snake-3-play.png)

Muốn xem mã nguồn? Bấm **Show code** ở góc trên Canvas để xem toàn bộ file `snake_game.html`. Bạn cũng có thể bấm nút **Download** (⬇) để tải project về máy, hoặc nút **▶** để chạy lại từ đầu.

![](images/chatgpt-snake-4-code.png)

::: tip 🌐 Khám phá thêm các công cụ AI Coding

Ngoài ChatGPT, bạn có thể thử các nền tảng AI Coding xuất sắc khác:
...
Ngoài ra, ta có thể yêu cầu LLM trực tiếp sinh prompt cấp project. Ở phần trước, ta tự viết prompt cho game Snake. Bây giờ thử bảo mô hình sinh một prompt có framework tổng thể và đường đi triển khai (bạn có thể dùng ChatGPT để sinh).
...
ChatGPT sẽ trả về một prompt chi tiết hơn, có cấu trúc rõ ràng (mục tiêu, tính năng, gợi ý UI/UX, gợi ý kỹ thuật...). Bạn copy prompt đó, mở một cuộc trò chuyện mới với ChatGPT rồi dán vào để nó sinh lại project hoàn chỉnh và ấn tượng hơn.
